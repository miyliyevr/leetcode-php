...
class Solution {
    ...
    function exist($board, $word) {
        for ($i = 0; $i < count($board); $i++) {
            for ($j = 0; $j < count($board[$i]); $j++) {
                if (substr($word, 0, 1) == $board[$i][$j]) {
                    $result = $this->dfs($i, $j, $word, $board);
    ...
    function dfs($i, $j, $word, $board) {
        if (!isset($board[$i][$j]) || strlen($word)==0 || $word[0] != $board[$i][$j]) {
            return false;
        }

        if ($word == $board[$i][$j]) return true;
...
If the solution is not fast enough could be "Time Limit Exceeded" :
...
Runtime: 116 ms, faster than 100.00% of PHP online submissions for Word Search.
Memory Usage: 21.4 MB, less than 33.33% of PHP online submissions for Word Search.